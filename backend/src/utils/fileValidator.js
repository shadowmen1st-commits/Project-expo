...
export const validateFileBuffer = (buffer, filename, mimetype, maxSizeBytes = DEFAULT_MAX_FILE_SIZE) => {
    // 1. Null check
    if (!buffer || buffer.length === 0) {
        throw new Error('FILE_EMPTY');
    }

    // 2. Check null byte in filename
    ...
    // 5. Double extension bypass check
    const parts = filename.split('.');
    if (parts.length > 2) {
        const riskyExtensions = ['.js', '.jsx', '.ts', '.tsx', '.html', '.htm', '.svg', '.sh', '.bat', '.exe', '.zip', '.tar', '.gz', '.php'];
    ...
    // 8. Magic bytes/file signature verification
