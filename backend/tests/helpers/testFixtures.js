...
export async function createApprovedWorker({category, profile={}, user={}}={}) { const worker=await createTestUser('WORKER',user); await WorkerProfile.create({userId:worker._id,serviceCategoryIds:category?[category._id]:[],verificationStatus:'APPROVED',hourlyRate:30000,dailyRate:200000,minimumBookingDuration:1,bufferMinutes:0,availability:[0,1,2,3,4,5,6].map(day=>({day,start:'00:00',end:'23:59',isWorking:true})),...profile}); return worker; }
