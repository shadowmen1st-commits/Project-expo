...
const FAQItem = ({ q, a }) => {
    ...
    return (
        <div 
            className={`border rounded-2xl overflow-hidden transition-all duration-300 cursor-pointer ${open ? 'border-[#EAB308] bg-[#FEFCE8]' : 'border-[#E7E0D8] bg-white hover:border-[#EAB308]/50'}`} 
            onClick={() => setOpen(o => !o)}
        >
            <div className="flex items-center justify-between px-6 py-4 gap-4">
                <span className="text-[#1C1917] font-semibold text-sm">{q}</span>
                <span className={`flex-shrink-0 w-6 h-6 rounded-full border flex items-center justify-center transition-all ${open ? 'border-[#EAB308] bg-[#EAB308]/10 rotate-45' : 'border-[#E7E0D8]'}`}>
        ...
        <div className="min-h-screen bg-[#FAF6F0] text-[#1C1917] overflow-x-hidden font-sans">
            ...
            <section className="pt-32 pb-12 px-6 relative overflow-hidden">
                <div className="pointer-events-none absolute inset-0">
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[350px] bg-[#EAB308]/10 blur-[120px] rounded-full"/>
                </div>
                ...
                <div className="relative max-w-3xl mx-auto text-center">
                    ...
                    <div className="inline-flex items-center gap-4 bg-white border border-[#E7E0D8] rounded-2xl p-1.5 shadow-sm">
                        <button onClick={() => setYearly(false)} className={`px-5 py-2 rounded-xl text-sm font-semibold transition-all cursor-pointer ${!yearly ? 'bg-[#EAB308] text-white shadow-sm' : 'text-[#78716C] hover:text-[#1C1917]'}`}>
                            Monthly
                        </button>
                        <button onClick={() => setYearly(true)} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold transition-all cursor-pointer ${yearly ? 'bg-[#EAB308] text-white shadow-sm' : 'text-[#78716C] hover:text-[#1C1917]'}`}>
                            Yearly
            ...
            <section className="pb-24 px-6">
                <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
                    {PLANS.map((plan) => (
                        <div 
                            key={plan.id} 
                            className={`relative flex flex-col rounded-3xl border p-8 transition-all duration-300 shadow-sm ${plan.highlight ? 'bg-white border-[#EAB308]' : 'bg-white border-[#E7E0D8] hover:border-[#EAB308]/50'}`}
                        >
                            {plan.badge && (
                                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-[11px] font-bold bg-[#EAB308] text-white shadow-sm">
                                    {plan.badge}
                                </div>
                            )}
                            ...
                            <ul className="space-y-3 mb-8 flex-1">
                                {plan.features.map((f, i) => (
                                    <li key={i} className={`flex items-center gap-3 text-sm ${f.included ? 'text-[#1C1917]' : 'text-[#A8A29E]'}`}>
                                        <span className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${f.included ? 'bg-[#FEFCE8] border border-[#FEF08A]' : 'bg-[#FAF6F0] border border-[#E7E0D8]'}`}>
                                            {f.included ? (
                                                <svg className="w-3 h-3 text-[#EAB308]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <section className="py-20 px-6 bg-[#F4EFE6] border-y border-[#E7E0D8]">
                <div className="max-w-5xl mx-auto">
                    <div className="text-center mb-12">
                        <p className="text-[#EAB308] text-sm font-semibold uppercase tracking-wider mb-3">For Professionals</p>
                    ...
                    <div className="overflow-hidden rounded-3xl border border-[#E7E0D8] bg-white shadow-sm">
                        ...
                        {COMMISSION_TIERS.map((row, i) => (
                            <div key={i} className={`grid grid-cols-4 items-center px-6 py-5 gap-4 transition-colors hover:bg-[#FEFCE8] ${i !== COMMISSION_TIERS.length - 1 ? 'border-b border-[#E7E0D8]' : ''}`}>
