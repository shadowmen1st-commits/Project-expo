...
export const SharedNavbar = () => {
    ...
    if (appActive) {
        ...
        return (
            <nav className="fixed top-0 left-0 right-0 z-50 bg-[#FAF6F0]/95 backdrop-blur-xl border-b border-[#E7E0D8] shadow-sm py-3">
    ...
    return (
        <>
            <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled
                ? 'bg-[#FAF6F0]/95 backdrop-blur-xl border-b border-[#E7E0D8] shadow-sm py-3'
                : 'bg-[#FAF6F0]/80 backdrop-blur-md border-b border-transparent py-5'}`}>
                <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                    {/* Logo */}
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        <button onClick={() => navigate('/login')} className="text-[#78716C] hover:text-[#1C1917] text-sm font-medium transition-colors px-3 py-1.5 cursor-pointer outline-none">
                            Sign In
                        </button>
                    ...
                    {/* Mobile menu button */}
                    <button onClick={() => setMobileMenuOpen(prev => !prev)} className="md:hidden text-[#78716C] hover:text-[#1C1917] focus:outline-none p-1.5 cursor-pointer">
                        {mobileMenuOpen ? (
            ...
            {/* Mobile menu overlay */}
            <div className={`fixed inset-0 z-40 bg-[#FAF6F0]/98 backdrop-blur-2xl transition-all duration-300 md:hidden ${mobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'}`}>
