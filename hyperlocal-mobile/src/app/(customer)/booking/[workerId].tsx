...
export default function CreateBookingScreen() {
      ...
      try {
        ...
        if (rawPending) {
          ...
          if (pending && (!pending.workerId || pending.workerId === canonicalParamId)) {
            ...
            if (user) {
              await storage.removeItem('SHADOWMEN_GUEST_PENDING_BOOKING');
  ...
  const handleConfirmBooking = async () => {
    ...
    if (!user) {
      ...
      await storage.setItem('SHADOWMEN_GUEST_PENDING_BOOKING', JSON.stringify(pending));
      router.push({
        pathname: '/(auth)/login',
        params: {
          redirect: 'booking',
    ...
    try {
      ...
      const payload = {
        ...
        addressSnapshot: {
          houseNumber: effectiveHouseNo || '1',
          street: effectiveStreet || 'Main Street',
          locality: landmark.trim() || effectiveStreet || 'Area',
          landmark: landmark.trim() || undefined,
          city: effectiveCity || 'New Delhi',
          state: (detectedState && detectedState.trim().length >= 2) ? detectedState.trim() : 'Delhi',
          pincode: effectivePincode || '110001',
          addressType: (addressType === 'OFFICE' || addressType === 'OTHER') ? addressType : 'HOME',
          instructions: instructions.trim() || undefined,
          latitude: Number(selectedLat ?? customerLat ?? 28.6139),
      ...
      // 2. Generate Payment Order ahead of time (non-blocking)
      try {
        ...
        console.warn('[BOOKING_SUBMIT] Non-fatal payment order pre-generation error:', orderErr);
      }

      // 3. Navigate directly to the dedicated Payment Screen
      console.log('[BOOKING_SUBMIT] Transitioning cleanly to Payment Screen for booking:', bookingId);
